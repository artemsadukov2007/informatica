...
#считаем буквы наверное
    letter_count = {}
    order = []  # список для сохранения порядка появления
    for char in text:
        if char.isalpha():
            lower_char = char.lower()
            if lower_char not in letter_count:
                order.append(lower_char)  # запоминаем порядок первого появления
            letter_count[lower_char] = letter_count.get(lower_char, 0) + 1
    return letter_count, order
...
letter_counts, order = count_letters(main_str)
